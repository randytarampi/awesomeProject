...
urlpatterns = patterns('scheduler.views',
    ...
    url(r'^courses/(?P<subject>[A-Z]+)/$', SubjectListView.as_view(), name = "scheduler_coursesSubject"),
    url(r'^courses/(?P<subject>[A-Z]+)/(?P<number>[X]?[X]?[0-9]+[A-Z]?)/$', SubjectNumberListView.as_view(), name = "scheduler_coursesSubjectNumber"),
